...
class com.scaleform.controls.MenuScrollingList extends ScrollingList {
...
// Initialization:
    ...
    private function changeFocus():Void {                    		
		...
		if (renderer != null) {    
            ...
            FocusItemRenderer(renderer).focusIndicator.gotoAndPlay( (_focused) ? "show" : "hide" );
            var OptionStepper:MovieClip = renderer["optionstepper"];
			if (OptionStepper != null)
			{
                OptionStepper.gotoAndPlay( (_focused) ? "focused" : "default" );
			}
		}		                       
	}
    ...
    /** Override for setState() that removes the _scrollBar.disabled call. */
    private function setState():Void {
		tabEnabled = focusEnabled = !_disabled;
		gotoAndPlay(_disabled ? "disabled" : _focused ? "focused" : "default");		
		if (_scrollBar) { 
            // Since _scrollBar is not a UIComponent, the .disabled call will fail.
